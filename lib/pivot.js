...
class Pivot extends Chain {
  ...
  async query(...args) {
    const sql = this.conn.mysql.format(...args);
    if (this.options.logging) this.options.logger.info(`superdb sql: \n${sqlFormatter.format(sql)}\n`);
  ...
  async deleteOne(tbName, options) {
    const tempOptions = Object.assign({}, options, { limit: 1 });
    return this.delete(tbName, tempOptions);
  }
}
