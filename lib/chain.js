...
class Chain {
  ...
  transferData(actionType, params) {
    ...
    this
      ...
      .ttl(params.ttl);
    if (!this.body.table) {
      throw new Error('superdb err: Must input tbName!');
    }
  ...
  mergeBodyData(source = {}) {
    const tempSource = _.merge({}, this.initialBody, source);
    // beforeHooks
    for (const [key, func] of Object.entries(this.options.beforeHooks)) {
      if (jsType('Function')(func)) tempSource[key] = func(tempSource[key]);
    }
    const result = _.merge({}, this.body, tempSource);
