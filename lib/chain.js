...
class Chain {
  ...
  join(param) {
    ...
    } else if (jsType('Object')(param) || jsType('String')(param)) {
      arr.push(param);
    } else {
      throw new Error('superdb err: Join function params must Object or Array or String!');
    }
    const newArr = arr.map((v) => {
      ...
      if (jsType('Object')(value)) {
        ...
        return `${value.direction ? `${value.direction} ` : ''}JOIN ${value.table} ON ${value.on} `;
      } if (jsType('String')(value)) {
        return value;
      }
      throw new Error('superdb err: Join function Array must contain String or Object');
    });
  ...
  mergeBodyData(source = {}) {
    // 首字母小写
    const firstLowerObj = Object.keys(source).reduce((c, k) => (c[k.charAt(0).toLowerCase() + k.slice(1)] = source[k], c), {});

    const result = _.merge({}, this.body, firstLowerObj); // 链式操作不可以与Options同时进行,对于Array类型数据来说，Merge会造成覆盖,或许我们需要的是concat
    // beforeHooks
